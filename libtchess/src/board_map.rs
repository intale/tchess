...
impl BoardMap {
    ...
    pub fn find_piece_by_id(&self, color: &Color, piece_id: &PieceId) -> &Rc<Piece> {
        self.maybe_find_piece_by_id(color, piece_id)
            .expect(format!("Couldn't find piece by id {}", piece_id).as_str())
    }

    pub fn maybe_find_piece_by_id(&self, color: &Color, piece_id: &PieceId) -> Option<&Rc<Piece>> {
        self.active_pieces[color].get(&Piece::UnknownPiece(*piece_id))
    }
}
