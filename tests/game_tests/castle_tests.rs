...
mod rook_gets_pinned {
    ...
    fn setup_board() -> Board {
        ...
        board.add_piece(
            "Rook", Color::Black, vec![], vec![], Point::new(1, 2)
        );
    ...
    fn expectation<T: PartialEq + Debug>() -> Expect<T, Board> {
        ...
        expectation.expect(|board| {
            ...
            assert!(
                ...
                "Unable to move {:?} on a1", enemy_rook
            );
            println!("{}", board.pp());
...
mod castle_is_possible {
    ...
    fn expectation<T: PartialEq + Debug>() -> Expect<T, Board> {
        ...
        expectation.expect(|board| {
            ...
            let castle_points = CastlePoints::new(
                ...
                Point::new(3, 1), Point::new(2, 1),
            );
            assert!(
                board.move_piece(&king, &PieceMove::Castle(castle_points)),
                "Could no perform queen-side castling using {:?}", king
            );
            println!("{}", board.pp());
    ...
    fn it_places_pieces_on_correct_points() {
        expectation().to_change(|board| {
            ...
            [king.current_position(), rook.current_position()]
        }).to(|_board| {
            [Point::new(3, 1), Point::new(4, 1)]
        });
    }
    ...
    fn it_does_not_allow_another_castle() {
        expectation().to_change(|board| {
            let king = board.find_piece_by_id(1).unwrap();
            board.moves(&Color::White).moves_of(&king).to_vec().clone_moves()
        }).to(|_board| {
            vec![
